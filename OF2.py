        ...
        try:
            if search_engine == "DuckDuckGo API" and DDGS_AVAILABLE:
                ...
                try:
                    ...
                    self.search_debug_info += f"Error: {error_msg}\nFalling back to HTML scraping method\n"
            ...
            if search_engine in ["DuckDuckGo", "Google"]:
                ...
                if search_engine == "DuckDuckGo":
                    # Use BeautifulSoup for parsing DuckDuckGo HTML results
                    soup = BeautifulSoup(content, 'html.parser')
                    result_divs = soup.find_all("div", class_="result__body")
                    self.search_debug_info += f"Found {len(result_divs)} result divs using BeautifulSoup\n\n"
                    for i, div in enumerate(result_divs[:max_results]):
                        ...
                        url = a_tag['href'] if a_tag and a_tag.has_attr('href') else "No URL"
                        snippet_tag = div.find("a", class_="result__snippet")
                        snippet = snippet_tag.get_text(strip=True) if snippet_tag else "No snippet"
                        search_results.append(f"Result {i + 1}:\nTitle: {title}\nSnippet: {snippet}\nURL: {url}\n")
                        self.search_debug_info += f"Result {i + 1} - {title}\n"
                elif search_engine == "Google":
                    # Basic Google search parsing using regex as fallback
                    result_divs = re.findall(r'<div class="g">(.*?)</div>\s*</div>\s*</div>', content, re.DOTALL)
                    self.search_debug_info += f"Found {len(result_divs)} result divs in HTML\n\n"
                    for i, div in enumerate(result_divs[:max_results]):
                        title_match = re.search(r'<h3 class=".*?">(.*?)</h3>', div, re.DOTALL)
                        title = html.unescape(re.sub(r'<.*?>', '', title_match.group(1))) if title_match else "No title"
                        snippet_match = re.search(r'<span class=".*?">(.*?)</span>', div, re.DOTALL)
                        snippet = html.unescape(re.sub(r'<.*?>', '', snippet_match.group(1))) if snippet_match else "No snippet"
    ...
    def _process_message(self, model, message):
        try:
            ...
            if self.web_search_enabled.get():
                ...
                self.root.after(0, lambda: self.progress_label.config(text="🤖 AI is processing search results..."))
...
Please answer the question based on the search results above. If the search results aren't relevant or don't contain the necessary information, use your knowledge to provide the best answer possible."""
    ...
    def _show_search_debug(self):
        self.chat_display.config(state=tk.NORMAL)
        self.chat_display.insert(tk.END, f"🔧 Search Debug Information:\n{self.search_debug_info}\n\n", "debug_info")
    ...
    def save_current_session(self):
        ...
        self.sessions[self.current_session_id]["updated_at"] = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
